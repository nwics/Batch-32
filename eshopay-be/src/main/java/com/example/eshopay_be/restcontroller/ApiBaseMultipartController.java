...
public abstract class ApiBaseMultipartController<T, ID> extends ApiBaseController<T, ID> {
    ...
    public abstract ResponseEntity<ApiResponse<T>> createMultipart(
            @RequestPart("data") T dto,
            @RequestPart(value = "file", required = false) MultipartFile file,
            @RequestParam(value = "description", required = false) String description);

    @GetMapping("/view/{filename:.+}")
    public abstract ResponseEntity<Resource> viewImage(@PathVariable String filename);
    ...
    public abstract ResponseEntity<ApiResponse<T>> updateMultipart(
            @PathVariable ID id,
            @RequestPart("data") T dto,
            @RequestPart(value = "file", required = false) MultipartFile file,
            @RequestParam(value = "description", required = false) String description);
