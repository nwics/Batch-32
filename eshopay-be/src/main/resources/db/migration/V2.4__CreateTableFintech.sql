...
-- Drop tabel dalam urutan yang benar (tabel child dulu, baru parent)
...
create table fintech.transactions(
   ...
   from_account bigint constraint fk_trac_from_account references fintech.accounts(account_id),
   to_account bigint constraint fk_trac_to_account references fintech.accounts(account_id),
   trac_noref varchar(25) null constraint fk_trac_ref references fintech.transactions(trac_no),
   debet decimal(15,2),
   credit decimal(15,2),
